...
class StudyRepository(
    ...
    suspend fun saveStudyScheduleFromProfile(profile: UserProfile) = withContext(Dispatchers.IO) {
        try {
            val blocksJson = profile.customStudyBlocksJson
            if (!blocksJson.isNullOrBlank() && blocksJson != "[]") {
                val array = org.json.JSONArray(blocksJson)
                for (i in 0 until array.length()) {
                    val obj = array.getJSONObject(i)
                    val title = obj.optString("subject", profile.subjects.firstOrNull() ?: "General Studies")
                    val startTime = obj.optString("startTime", "07:00 PM")
                    val duration = obj.optInt("durationMinutes", 60)
                    ...
                    if (isEnabled) {
                        val scheduleItem = StudyScheduleItem(
                            ...
                            subject = title,
                            topic = obj.optString("topic", "Focused Study"),
                            isAutoFocus = true,
                            isStrictMode = false,
                            repeatType = "DAILY",
                            repeatDaysJson = "[\"MON\",\"TUE\",\"WED\",\"THU\",\"FRI\",\"SAT\",\"SUN\"]",
                            reminderMinutesBefore = 10,
                            isPaused = false
                        )
                        database.studyScheduleDao().insertOrUpdateScheduleItem(scheduleItem)
                    }
                }
            } else {
                // If custom blocks are empty, generate initial item based on preferredStudyStartTime & subjects
                val primarySubject = profile.subjects.firstOrNull() ?: "Core Preparation"
                val startTime = profile.preferredStudyStartTime.ifBlank { "06:00 PM" }
                val duration = (profile.dailyTargetMinutes / 2).coerceIn(30, 90)
                val defaultSchedule = StudyScheduleItem(
                ...
                database.studyScheduleDao().insertOrUpdateScheduleItem(defaultSchedule)
            }
        } catch (e: Exception) {
            android.util.Log.e("StudyRepository", "Failed to save study schedule from profile", e)
        }
    }
